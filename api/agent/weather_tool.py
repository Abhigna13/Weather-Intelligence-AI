...
def predict_temperature(values):
    ...
    Converts the 13 weather parameters into the
    named fields expected by the FastAPI endpoint.
    """
    ...
    payload = {
        "summary": values[0],
        "precip_type": values[1],
        "apparent_temperature": values[2],
        "humidity": values[3],
        "wind_speed": values[4],
        "wind_bearing": values[5],
        "visibility": values[6],
        "cloud_cover": values[7],
        "pressure": values[8],
        "daily_summary": values[9],
        "year": values[10],
        "month": values[11],
        "day": values[12]
    }
